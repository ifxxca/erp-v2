...
import './App.css'
import {
  ApiError, apiRequest, type ChecklistTemplate, type LoginResponse, type OperationsContext, type Page,
...
function App() {
  ...
  return <AppShell header={{ height: 72 }} padding="md">
    ...
    {checkoutOpened && <CheckoutModal opened onClose={checkoutModal.close} vehicles={vehicles} checklist={checklist} onSubmit={async (values) => { await apiRequest(`${base}/fleet/trips/checkout`, { method: 'POST', body: JSON.stringify(values) }, token); checkoutModal.close(); await loadData() }} reportError={reportError} />}
...
function CheckoutModal({ opened, onClose, onSubmit, reportError, vehicles, checklist }: ModalProps<Record<string, unknown>> & { vehicles: Vehicle[]; checklist: ChecklistTemplate | null }) {
  ...
  return <Modal opened={opened} onClose={onClose} title="Checkout kendaraan" size="lg"><form onSubmit={form.onSubmit(async (values) => { if (!checklist) return; setBusy(true); try { await onSubmit({ ...values, destination: values.destination || null, answers: checklist.items.map((item) => ({ item_id: item.id, result: answers[item.id] })) }); form.reset() } catch (cause) { reportError(cause) } finally { setBusy(false) } })}><Stack><Select searchable label="Kendaraan tersedia" required data={available.map((vehicle) => ({ value: vehicle.id, label: `${vehicle.plate_number} · ${vehicle.brand} ${vehicle.model} · ${vehicle.current_odometer.toLocaleString('id-ID')} km` }))} value={form.values.vehicle_id} onChange={(value) => { form.setFieldValue('vehicle_id', value ?? ''); const selected = vehicles.find((vehicle) => vehicle.id === value); if (selected) form.setFieldValue('start_odometer', selected.current_odometer) }} /><Group grow><TextInput label="Keperluan" required {...form.getInputProps('purpose')} /><TextInput label="Tujuan" {...form.getInputProps('destination')} /></Group><NumberInput label="Odometer keluar (km)" required min={0} thousandSeparator="." decimalSeparator="," {...form.getInputProps('start_odometer')} /><Divider label={checklist?.name ?? 'Checklist belum tersedia'} />{checklist?.items.map((item) => <Group key={item.id} justify="space-between" wrap="nowrap"><Box><Text size="sm" fw={600}>{item.label}</Text><Text size="xs" c={item.is_critical ? 'red' : 'dimmed'}>{item.is_critical ? 'Item kritis · wajib lulus' : 'Pemeriksaan wajib'}</Text></Box><Select w={150} value={answers[item.id] ?? 'pass'} onChange={(value) => value && setAnswers((current) => ({ ...current, [item.id]: value }))} data={[{ value: 'pass', label: 'Baik' }, { value: 'fail', label: 'Bermasalah' }, { value: 'not_applicable', label: 'Tidak berlaku' }]} /></Group>)}<Button type="submit" loading={busy} disabled={!checklist || !available.length}>Konfirmasi checkout</Button></Stack></form></Modal>
}
