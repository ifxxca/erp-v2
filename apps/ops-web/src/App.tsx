...
import './App.css'
import {
  ApiError, cancelVehicleTrip, challengeOperationsMfa, changeVehicleStatus, checkInVehicle, checkoutVehicle,
  createVehicle, createVehicleType, createWorkOrder, downloadEvidence, getChecklistTemplate, getOperationsContexts,
  getVehicleTrip, listVehicles, listVehicleTrips, listVehicleTypes, listWorkOrders, loginOperations,
  logoutOperations, transitionWorkOrder, uploadChecklistEvidence,
  type CancelVehicleTripInput, type CheckInVehicleInput, type ChecklistTemplate, type CheckoutVehicleInput,
  type CreateVehicleInput, type CreateVehicleTypeInput, type CreateWorkOrderInput, type EvidenceFile,
  type OperationsContext, type Vehicle, type VehicleStatus, type VehicleTrip, type VehicleType, type WorkOrder,
...
function App() {
  ...
  const scope = useMemo(() => activeContext ? {
    companyId: activeContext.company.id,
    locationId: activeContext.location.id,
  } : null, [activeContext])
  ...
  const resetSession = useCallback(() => {
    ...
    setBusy(false)
    setVehicles([]); setVehicleTypes([]); setWorkOrders([]); setTrips([]); setChecklist(null)
  }, [])
  ...
  useEffect(() => {
    ...
    getOperationsContexts(token).then((response) => {
      setContexts(response)
      setContextKey((current) => current || (response[0] ? `${response[0].company.id}:${response[0].location.id}` : ''))
      setStage('ready')
    }).catch(reportError)
  }, [reportError, stage, token])

  const loadData = useCallback(async () => {
    if (!scope || !activeContext) return
    const revision = ++loadRevision.current
    setBusy(true)
    try {
      const [vehiclePage, types, workOrderPage, tripPage, template] = await Promise.all([
        activeContext.capabilities.can_view_vehicles ? listVehicles(scope, token) : Promise.resolve(null),
        activeContext.capabilities.can_view_vehicles ? listVehicleTypes(scope, token) : Promise.resolve(null),
        activeContext.capabilities.can_view_work_orders ? listWorkOrders(scope, token) : Promise.resolve(null),
        activeContext.capabilities.can_view_trips ? listVehicleTrips(scope, token) : Promise.resolve(null),
        activeContext.capabilities.can_operate_trips ? getChecklistTemplate(scope, token) : Promise.resolve(null),
      ])
      if (loadRevision.current !== revision) return
      setVehicles(vehiclePage?.data ?? [])
      setVehicleTypes(types ?? [])
      setWorkOrders(workOrderPage?.data ?? [])
      setTrips(tripPage?.data ?? [])
      setChecklist(template)
    } catch (cause) {
      if (loadRevision.current === revision) reportError(cause)
    } finally {
      if (loadRevision.current === revision) setBusy(false)
    }
  }, [activeContext, reportError, scope, token])
  useEffect(() => { if (stage === 'ready') void loadData() }, [loadData, stage])

  async function logout() { try { await logoutOperations(token) } finally { resetSession() } }
  ...
  if (!activeContext || !scope) return <Center mih="100vh"><Card maw={460}><Stack><Title order={2}>Belum ada area kerja</Title><Text c="dimmed">Akun ini belum memiliki akses Fleet atau Maintenance pada lokasi aktif.</Text><Button onClick={logout}>Keluar</Button></Stack></Card></Center>
    ...
    <AppShell.Main><Container size="xl"><Stack gap="lg">
      ...
      <Tabs value={workspace} onChange={(value) => setWorkspace(value as Workspace)}>
        ...
        <Tabs.Panel value="maintenance" pt="lg"><MaintenancePanel orders={workOrders} busy={busy} canManage={activeContext.capabilities.can_manage_work_orders} onAdd={workOrderModal.open} onTransition={async (order, status) => {
          try { await transitionWorkOrder(scope, order.id, { status, note: ['completed', 'cancelled'].includes(status) ? `Work order ${status} from Operations.` : null }, token); await loadData() } catch (cause) { reportError(cause) }
        }} /></Tabs.Panel>
      </Tabs>
    </Stack></Container></AppShell.Main>
    <VehicleModal opened={vehicleOpened} onClose={vehicleModal.close} types={vehicleTypes} onSubmit={async (values) => { await createVehicle(scope, values, token); vehicleModal.close(); await loadData() }} reportError={reportError} />
    <VehicleTypeModal opened={typeOpened} onClose={typeModal.close} onSubmit={async (values) => { await createVehicleType(scope, values, token); typeModal.close(); await loadData() }} reportError={reportError} />
    <WorkOrderModal opened={workOrderOpened} onClose={workOrderModal.close} vehicles={vehicles} onSubmit={async (values) => { await createWorkOrder(scope, values, token); workOrderModal.close(); await loadData() }} reportError={reportError} />
    {checkoutOpened && <CheckoutModal opened onClose={checkoutModal.close} companyId={activeContext.company.id} token={token} vehicles={vehicles} checklist={checklist} onSubmit={async (values) => { await checkoutVehicle(scope, values, token); checkoutModal.close(); await loadData() }} reportError={reportError} />}
    {checkinOpened && <CheckinModal opened onClose={checkinModal.close} trip={selectedTrip} onSubmit={async (values) => { if (!selectedTrip) return; await checkInVehicle(scope, selectedTrip.id, values, token); checkinModal.close(); setSelectedTrip(null); await loadData() }} reportError={reportError} />}
    {cancelOpened && <CancelTripModal opened onClose={cancelModal.close} trip={selectedTrip} onSubmit={async (values) => { if (!selectedTrip) return; await cancelVehicleTrip(scope, selectedTrip.id, values, token); cancelModal.close(); setSelectedTrip(null); await loadData() }} reportError={reportError} />}
...
function Login({ onAuthenticated }: { onAuthenticated: (token: string, mfa: boolean) => void }) {
  ...
  return <Center mih="100vh" p="md" className="auth-bg"><Card w="100%" maw={420} p="xl"><form onSubmit={form.onSubmit(async (values) => { setBusy(true); try { const response = await loginOperations(values.email, values.password); onAuthenticated(response.access_token, response.mfa_required) } catch (cause) { notifications.show({ color: 'red', title: 'Login gagal', message: cause instanceof Error ? cause.message : 'Periksa kredensial.' }) } finally { setBusy(false) } })}><Stack><Box className="brand-mark"><IconTool /></Box><Title order={2}>Masuk ke Operations</Title><Text c="dimmed" size="sm">Gunakan akun perusahaan dan area kerja yang sudah diberikan.</Text><TextInput label="Email" type="email" required {...form.getInputProps('email')} /><PasswordInput label="Password" required {...form.getInputProps('password')} /><Button type="submit" loading={busy}>Masuk</Button></Stack></form></Card></Center>
}
function Mfa({ token, onVerified, onCancel }: { token: string; onVerified: () => void; onCancel: () => void }) {
  ...
  return <Center mih="100vh" p="md"><Card w="100%" maw={420} p="xl"><Stack><Title order={2}>Verifikasi MFA</Title><Text c="dimmed">Masukkan kode authenticator atau recovery code.</Text><TextInput label="Kode" value={code} onChange={(event) => setCode(event.currentTarget.value)} /><Button loading={busy} onClick={async () => { setBusy(true); try { await challengeOperationsMfa(token, code); onVerified() } catch (cause) { notifications.show({ color: 'red', title: 'Verifikasi gagal', message: cause instanceof Error ? cause.message : 'Kode tidak valid.' }) } finally { setBusy(false) } }}>Verifikasi</Button><Button variant="subtle" color="gray" onClick={onCancel}>Batal</Button></Stack></Card></Center>
}
...
function MaintenancePanel({ orders, busy, canManage, onAdd, onTransition }: { orders: WorkOrder[]; busy: boolean; canManage: boolean; onAdd: () => void; onTransition: (order: WorkOrder, status: WorkOrderTransitionInput['status']) => void }) {
...
function CheckoutModal({ opened, onClose, onSubmit, reportError, companyId, token, vehicles, checklist }: ModalProps<CheckoutVehicleInput> & { companyId: string; token: string; vehicles: Vehicle[]; checklist: ChecklistTemplate | null }) {
  const form = useForm({ initialValues: { vehicle_id: '', purpose: '', destination: '', start_odometer: 0 } })
  const [answers, setAnswers] = useState<Record<string, CheckoutVehicleInput['answers'][number]['result']>>(() => Object.fromEntries(
...
function CheckinModal({ opened, onClose, onSubmit, reportError, trip }: ModalProps<CheckInVehicleInput> & { trip: VehicleTrip | null }) {
...
function CancelTripModal({ opened, onClose, onSubmit, reportError, trip }: ModalProps<CancelVehicleTripInput> & { trip: VehicleTrip | null }) {
...
function VehicleModal({ opened, onClose, onSubmit, reportError, types }: ModalProps<CreateVehicleInput> & { types: VehicleType[] }) {
  const form = useForm<CreateVehicleInput>({ initialValues: { vehicle_type_id: '', code: '', plate_number: '', brand: '', model: '', model_year: new Date().getFullYear(), ownership_type: 'owned', provider_name: '', current_odometer: 0 } }); const [busy, setBusy] = useState(false)
...
function VehicleTypeModal({ opened, onClose, onSubmit, reportError }: ModalProps<CreateVehicleTypeInput>) {
...
function WorkOrderModal({ opened, onClose, onSubmit, reportError, vehicles }: ModalProps<CreateWorkOrderInput> & { vehicles: Vehicle[] }) {
  const form = useForm({ initialValues: { vehicle_id: '', work_order_date: new Date().toISOString().slice(0, 10), priority: 'normal' as CreateWorkOrderInput['priority'], problem_description: '', parts_cost: 0, job_description: '', labor_cost: 0 } }); const [busy, setBusy] = useState(false)
